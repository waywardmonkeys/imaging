...
//!
//! This crate provides a small [`imaging::PaintSink`] implementation that can replay an
//! [`imaging::record::Scene`] as an SVG document.
//!
...
use imaging::{
    ...
    StrokeRef,
    record::{Glyph, Scene, ValidateError, replay},
};
use kurbo::{Affine, BezPath, Rect};
use peniko::{BlendMode, Brush, Color, Compose, Extend, Fill, GradientKind, Mix, Style};
...
impl SvgRenderer {
    ...
    fn write_clip_def(&mut self, id: &str, clip: &ClipRef<'_>) {
        let _ = write!(
            self.defs,
        ...
        match clip {
            ClipRef::Fill {
                ...
                let mut path = self.geometry_to_path(shape.clone());
    ...
    fn write_path_with_attrs(
        ...
        if let Some(css) = blend_mode_css(blend) {
            let _ = write!(style_attr, " style=\"mix-blend-mode:{css}\"");
        }
        match fill_rule {
            Some(rule) => {
                let _ = write!(
                    self.body,
                    "<path d=\"{d}\" fill-rule=\"{}\"{paint_attrs}{style_attr}/>",
                    fill_rule_svg(rule)
                );
            }
            None => {
                let _ = write!(self.body, "<path d=\"{d}\"{paint_attrs}{style_attr}/>");
            }
        }
    }

    fn glyph_placeholder_path(&self, draw: &GlyphRunRef<'_>, glyph: Glyph) -> BezPath {
        let font_size = f64::from(draw.font_size.max(1.0));
        let rect = Rect::new(
            f64::from(glyph.x),
            f64::from(glyph.y) - font_size,
            f64::from(glyph.x) + font_size * 0.6,
            f64::from(glyph.y),
        );
        let mut path = self.geometry_to_path(GeometryRef::Rect(rect));
        if let Some(glyph_transform) = draw.glyph_transform {
            path.apply_affine(glyph_transform);
        }
        path.apply_affine(draw.transform);
        path
    }
}

impl PaintSink for SvgRenderer {
    ...
    fn fill(&mut self, draw: FillRef<'_>) {
        let mut path = self.geometry_to_path(draw.shape.clone());
        path.apply_affine(draw.transform);

        let brush = draw.brush.to_owned().multiply_alpha(draw.composite.alpha);
        let paint_attrs = self.style_for_brush(brush, PaintKind::Fill, draw.brush_transform);
        self.write_path_with_attrs(
            &path,
            &paint_attrs,
            Some(draw.fill_rule),
    ...
    fn stroke(&mut self, draw: StrokeRef<'_>) {
        ...
        let brush = draw.brush.to_owned().multiply_alpha(draw.composite.alpha);
        let mut paint_attrs = self.style_for_brush(brush, PaintKind::Stroke, draw.brush_transform);
        paint_attrs.push_str(&stroke_style_attrs(draw.stroke));
        self.write_path_with_attrs(&path, &paint_attrs, None, draw.composite.blend);
    }
    ...
    fn glyph_run(&mut self, draw: GlyphRunRef<'_>, glyphs: &mut dyn Iterator<Item = Glyph>) {
        ...
        match draw.style {
            Style::Fill(fill_rule) => {
                let paint_attrs = self.style_for_brush(brush, PaintKind::Fill, None);
                for glyph in glyphs {
                    let path = self.glyph_placeholder_path(&draw, glyph);
                    self.write_path_with_attrs(
                        &path,
                        &paint_attrs,
                        Some(*fill_rule),
            ...
            Style::Stroke(stroke) => {
                let mut paint_attrs = self.style_for_brush(brush, PaintKind::Stroke, None);
    ...
    fn blurred_rounded_rect(&mut self, draw: BlurredRoundedRect) {
        let id = self.next_filter_id();
        let filters = [Filter::blur_xy(
            f64_to_f32(draw.std_dev),
            f64_to_f32(draw.std_dev),
        )];
        self.write_filter_def(&id, &filters, draw.transform);

        let paint_attrs = style_for_solid_color(
            draw.color.multiply_alpha(draw.composite.alpha),
            PaintKind::Fill,
        );
        let mut attrs = String::new();
        let [x0, y0, x1, y1] = [draw.rect.x0, draw.rect.y0, draw.rect.x1, draw.rect.y1];
        let x = x0.min(x1);
        let y = y0.min(y1);
        let width = (x1 - x0).abs();
        let height = (y1 - y0).abs();
        let radius = draw.radius.max(0.0);

        let _ = write!(
            attrs,
            " x=\"{}\" y=\"{}\" width=\"{}\" height=\"{}\" rx=\"{}\" ry=\"{}\" filter=\"url(#{id})\"",
            fmt_f64_to_f32(x),
            fmt_f64_to_f32(y),
            fmt_f64_to_f32(width),
            fmt_f64_to_f32(height),
            fmt_f64_to_f32(radius),
            fmt_f64_to_f32(radius),
        );
        if draw.transform != Affine::IDENTITY {
            let _ = write!(
                attrs,
                " transform=\"{}\"",
                affine_to_svg_matrix(draw.transform)
            );
        }
        if let Some(css) = blend_mode_css(draw.composite.blend) {
            let _ = write!(attrs, " style=\"mix-blend-mode:{css}\"");
        }

        let _ = write!(self.body, "<rect{attrs}{paint_attrs}/>");
    }
}
...
fn clip_transform(clip: &ClipRef<'_>) -> Affine {
    match clip {
        ClipRef::Fill { transform, .. } | ClipRef::Stroke { transform, .. } => *transform,
    }
}

fn fill_rule_svg(rule: Fill) -> &'static str {
    match rule {
...
fn stroke_style_attrs(stroke: &kurbo::Stroke) -> String {
    use kurbo::{Cap, Join};
...
#[cfg(test)]
mod tests {
    use super::{SvgRenderer, bez_path_to_svg_d};
    use alloc::{format, string::String, vec};
    use imaging::{
        Composite, Filter, GroupRef, MaskMode, Painter,
        record::{Glyph, Mask, Scene},
    };
    use kurbo::{Rect, Shape, Stroke};
    use peniko::{
        Blob, Brush, Color, Compose, Extend, Fill, FontData, Gradient, ImageAlphaType, ImageBrush,
        ImageData, ImageFormat, Mix, Style,
    };

    fn render(scene: &Scene) -> String {
        SvgRenderer::new()
            .render_scene(scene, 64, 48)
            .expect("render svg")
    }

    #[test]
    fn exports_fill_and_stroke_paths() {
        ...
        assert!(svg.contains("<path d=\"M1 2L11 2L11 12L1 12Z\""));
        assert!(svg.contains(" fill=\"#ff0000\""));
        assert!(svg.contains(" stroke=\"#0000ff\" fill=\"none\""));
        assert!(svg.contains(" stroke-width=\"2\""));
    }

    #[test]
    fn exports_linear_gradient_defs() {
        let gradient = Gradient::new_linear((0.0, 0.0), (16.0, 0.0))
            .with_extend(Extend::Repeat)
            .with_stops([
                peniko::ColorStop::from((0.0, Color::BLACK)),
                peniko::ColorStop::from((1.0, Color::WHITE)),
            ]);
        let mut scene = Scene::new();
        let mut painter = Painter::new(&mut scene);
        painter
            .fill(Rect::new(0.0, 0.0, 16.0, 16.0), &gradient)
            .draw();
    ...
    #[test]
    fn approximates_sweep_gradients_with_linear_defs() {
        let gradient = Gradient::new_sweep((8.0, 8.0), 0.0, core::f32::consts::TAU).with_stops([
            peniko::ColorStop::from((0.0, Color::from_rgb8(0xff, 0x00, 0x00))),
            peniko::ColorStop::from((1.0, Color::from_rgb8(0x00, 0x00, 0xff))),
        ]);
        let mut scene = Scene::new();
        let mut painter = Painter::new(&mut scene);
        painter
    ...
    #[test]
    fn image_brushes_export_as_placeholders() {
        let image = ImageData {
            data: Blob::from(vec![255, 0, 255, 255]),
        ...
        let mut scene = Scene::new();
        let mut painter = Painter::new(&mut scene);
        painter.fill(Rect::new(0.0, 0.0, 8.0, 8.0), &brush).draw();

        let svg = render(&scene);
        assert!(svg.contains(" fill=\"#ff00ff\" fill-opacity=\"0.25\""));
    }

    #[test]
    fn glyph_runs_export_as_placeholder_paths() {
        let font = FontData::new(Blob::from(vec![0_u8]), 0);
        let glyphs = [Glyph {
            id: 7,
            x: 3.0,
            y: 14.0,
        ...
        let mut painter = Painter::new(&mut scene);
        painter
            .glyphs(&font, Color::BLACK)
            .font_size(10.0)
            .draw(&Style::Fill(Fill::NonZero), glyphs);

        let svg = render(&scene);
        let expected_path = bez_path_to_svg_d(&Rect::new(3.0, 4.0, 9.0, 14.0).to_path(0.1));
        assert!(svg.contains(&format!("<path d=\"{expected_path}\"")));
        assert!(!svg.contains("<text"));
    }

    #[test]
    fn masked_groups_are_currently_ignored() {
        let mask_scene = {
            let mut scene = Scene::new();
            let mut painter = Painter::new(&mut scene);
            painter
                .fill(Rect::new(0.0, 0.0, 8.0, 8.0), Color::WHITE)
                .draw();
            scene
        };
        let mask = Mask::new(MaskMode::Alpha, mask_scene);

        let mut scene = Scene::new();
        let mut painter = Painter::new(&mut scene);
        painter.with_group(GroupRef::new().with_mask(mask.as_ref()), |p| {
            p.fill(
        ...
        let svg = render(&scene);
        assert!(svg.contains("<g style=\"isolation:isolate\">"));
        assert!(svg.contains(" fill=\"#008000\""));
        assert!(!svg.contains("<mask"));
    }

    #[test]
    fn exports_group_clip_filter_and_blend_css() {
        let filters = [Filter::blur_xy(2.0, 3.0)];
        let mut scene = Scene::new();
        let mut painter = Painter::new(&mut scene);
        let group = GroupRef::new()
            .with_clip(imaging::ClipRef::fill(Rect::new(2.0, 3.0, 10.0, 11.0)))
            .with_filters(&filters)
            .with_composite(Composite::new(
                peniko::BlendMode::new(Mix::Multiply, Compose::SrcOver),
                0.5,
            ));
        painter.with_group(group, |p| {
            p.fill(Rect::new(0.0, 0.0, 12.0, 12.0), Color::WHITE).draw();
        });

        let svg = render(&scene);
        assert!(svg.contains("<clipPath id=\"clip1\""));
        assert!(svg.contains("<filter id=\"filter1\""));
        assert!(svg.contains("<feGaussianBlur"));
        assert!(svg.contains("clip-path=\"url(#clip1)\""));
        assert!(svg.contains("filter=\"url(#filter1)\""));
